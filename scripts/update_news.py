...
# Fontes PT verificadas: evitamos BBC/Club of Mozambique no feed PT porque publicam frequentemente em inglês.
RSS_PT = {
    'Mundo': [
...
THEMES = ['energy','energia','lng','gas','oil','petróleo','petroleo','econom','economia','investment','investimento','business','negócios','trade','market','artificial intelligence','inteligência artificial','technology','tecnologia','security','segurança','conflict','conflito','war','guerra','military','sanction','sanções','infrastructure','infraestrutura','jobs','emprego','mining','mineração','climate','drought','flood','water','água','interest rate','inflation','eleição','eleições','governo','presidente']
GLOBAL = ['china','united states','u.s.','usa','europe','european union','russia','ukraine','middle east','israel','iran','saudi','yemen','india','japan','south korea','nato','opec','world bank','imf','onu','estados unidos','união europeia','rússia','ucrânia']
AFRICA = ['africa','africano','africana','moçambique','mozambique','áfrica do sul','south africa','angola','tanzania','malawi','zambia','zimbabwe','kenya','nigeria','ghana','ethiopia','congo','rwanda','uganda','somalia','sudan','egypt','algeria','morocco','senegal','namibia','botswana','djibouti','guiné','cabo verde']
...
EN_WORDS = [' the ', ' and ', ' of ', ' to ', ' for ', ' with ', ' says ', ' from ', ' ai regulation ', " africa's "]
...
def likely_portuguese(title, desc):
    return not any(x in (' ' + title + ' ' + desc).lower() for x in EN_WORDS)

def relevant(sec, title, desc):
    t = (title + ' ' + desc).lower()
    if any(x in t for x in BAD) or not any(x in t for x in THEMES): return False
    if sec == 'Mundo': return any(x in t for x in GLOBAL) or any(x in t for x in ['lng','gas','oil','energy','artificial intelligence','war','guerra','sanction','tariff','interest rate','inflation','eleição','eleições'])
...
def parse(url, sec, source, hours, pt=True):
    r = urlopen(Request(url, headers={'User-Agent':'Mozilla/5.0 BriefingDiario/11'}), timeout=25)
    f = feedparser.parse(r.read()); out=[]
    for i in f.entries:
...
def build(lang):
    ...
    for s in TARGET:
        for src,u in feeds[s]:
            ...
            except Exception as e: print('feed',src,e)
        for q in queries[s]:
    ...
    if sum(map(len,by.values()))<MIN_NEWS:
        for s in TARGET:
            for src,u in feeds[s]:
                ...
                except Exception as e: print('fallback',src,e)
            for q in queries[s]:
                try: add(by[s],seen,google(s,q,FALLBACK_HOURS,pt))
                except Exception as e: print('google fallback',q,e)
    selected=[]
