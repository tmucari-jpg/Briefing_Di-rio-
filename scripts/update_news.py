...
# Fontes PT verificadas: RTP disponibiliza RSS por secção; DW mantém feed em português.
RSS_PT={
 'Mundo':[
 ...
 'África':[
   ('DW Português — África','https://rss.dw.com/syndication/feeds/DW_para_A_Verdade.12133-cb.html'),
   ('RTP Notícias — Mundo','https://www.rtp.pt/noticias/rss/mundo')
 ],
 'Moçambique':[
   ('DW Português — África','https://rss.dw.com/syndication/feeds/DW_para_A_Verdade.12133-cb.html'),
...
GLOBAL=['china','united states','europe','european union','russia','ukraine','middle east','israel','iran','saudi','yemen','india','japan','south korea','nato','opec','world bank','imf','onu']
AFRICA=['africa','africano','africana','moçambique','mozambique','áfrica do sul','angola','tanzânia','malawi','zâmbia','zimbabué','zimbabwe','quénia','nigéria','ghana','etiópia','congo','ruanda','uganda','somália','sudão','egito','argélia','marrocos','senegal','namíbia','botswana']
MOZ=['moçambique','mozambique','maputo','cabo delgado','pemba','niassa','nampula','beira','nacala','tete','inhambane','gaza','manica','zambézia','rovuma']
...
def relevant(sec,title,desc):
 ...
 if sec=='Mundo': return any(x in t for x in GLOBAL) or any(x in t for x in ['lng','gás','gas','energia','inteligência artificial','guerra','sanções','tarifas','inflação'])
...
def tags(t):
 ...
 if any(x in t for x in ['security','segurança','conflict','conflito','war','guerra','military','sanction','sanções']): o.append('Segurança')
 return o or ['Economia']
def context(sec,t):
 t=t.lower()
 if any(x in t for x in ['lng','gás','gas','oil','energy','energia']): return 'Energia e investimento são temas estratégicos para a região e podem alterar decisões de empresas e governos.','Pode influenciar investimento, custos de energia, fornecedores locais e receitas ligadas ao setor energético.'
 ...
 if any(x in t for x in ['war','guerra','conflict','conflito','security','segurança','military','sanction','sanções']): return 'A evolução tem relevância geopolítica e pode alterar riscos, comércio e decisões de investimento.','Pode afetar preços, cadeias de abastecimento, comércio regional e perceção de risco para investidores.'
 if any(x in t for x in ['econom','investment','investimento','business','negócios','trade','market','inflation','inflação']): return 'A notícia ajuda a perceber a direção da economia e das decisões de investimento.','Pode refletir-se em preços, acesso a capital, procura por fornecedores e oportunidades de negócio.'
...
def parse(url,sec,source,hours,pt=True):
 req=Request(url,headers={'User-Agent':'Mozilla/5.0 BriefingDiario/11','Accept':'application/rss+xml, application/xml, text/xml, */*'})
