...
RSS = {
    'pt': {
        'Mundo': [('BBC PT', 'https://feeds.bbci.co.uk/portuguese/international/rss.xml'), ('BBC World', 'https://feeds.bbci.co.uk/news/world/rss.xml')],
        'África': [('BBC PT', 'https://feeds.bbci.co.uk/portuguese/topics/africa/rss.xml'), ('BBC Africa', 'https://feeds.bbci.co.uk/news/world/africa/rss.xml')],
...
def published_dt(item):
    ...
    for key in ('published','updated'):
        ...
        if v:
            try:
                dt=parsedate_to_datetime(v); return dt.astimezone(timezone.utc) if dt.tzinfo else dt.replace(tzinfo=timezone.utc)
            except Exception: pass
    return None

def age_hours(dt): return (datetime.now(timezone.utc)-dt).total_seconds()/3600 if dt else 9999
def tags(text):
    t=text.lower(); out=[]
...
def relevant(section,title,desc):
    ...
    if not any(x in t for x in THEMES) or any(x in t for x in BAD): return False
...
def why_impact(section,text,lang):
    ...
    if any(x in t for x in ['artificial intelligence','inteligência artificial','technology','tecnologia',' ai ']): return (('Pode acelerar ou travar investimento, regulação e adoção tecnológica.','Para Moçambique, interessa por produtividade, emprego digital, serviços públicos e novas oportunidades de negócio.') if lang=='pt' else ('It can accelerate or slow investment, regulation and technology adoption.','For Mozambique, it matters for productivity, digital jobs, public services and new business opportunities.'))
    if any(x in t for x in ['war','guerra','conflict','conflito','security','segurança','military','sanction']): return (('Pode alterar riscos geopolíticos, rotas comerciais, energia e custos de financiamento.','Moçambique pode sentir efeitos através de importações, energia, comércio regional e investimento.') if lang=='pt' else ('It can change geopolitical risks, trade routes, energy and financing costs.','Mozambique may feel effects through imports, energy, regional trade and investment.'))
    if section=='Moçambique': return (('É uma evolução diretamente ligada ao ambiente económico, social ou institucional do país.','Pode afetar investimento, emprego, fornecedores locais, custos ou previsibilidade para empresas e famílias.') if lang=='pt' else ('It is directly linked to Mozambique’s economic, social or institutional environment.','It may affect investment, jobs, local suppliers, costs or predictability for businesses and households.'))
...
def parse_feed(url,section,lang,source,hours):
    req=Request(url,headers={'User-Agent':'Mozilla/5.0 (compatible; BriefingDiario/8.0)'})
...
def google(section,query,lang,hours):
    cfg=LANGS[lang]; q=query if hours<=24 else query.replace(' when:1d','')
...
def build(lang):
    by={s:[] for s in TARGET}; seen_by={s:set() for s in TARGET}; errors=[]
    queries={
        'Mundo':['oil LNG energy geopolitics AI trade sanctions markets when:1d','global economy interest rates investment technology conflict when:1d'],
        'África':['Africa energy investment infrastructure security technology economy when:1d','Africa LNG mining trade jobs projects when:1d'],
        'Moçambique':['Mozambique Moçambique LNG energy investment projects jobs economy security when:1d','Mozambique gas mining infrastructure business trade jobs when:1d']
    }
    for section in TARGET:
        seen=seen_by[section]
        for source,url in RSS.get(lang,{}).get(section,[]):
            try: add_unique(by[section],parse_feed(url,section,lang,source,PRIMARY_HOURS),seen,30)
            except Exception as e: errors.append(f'{section}/{source}: {type(e).__name__}')
        for q in queries[section]:
            try: add_unique(by[section],google(section,q,lang,PRIMARY_HOURS),seen,30)
    ...
    if sum(len(v) for v in by.values())<10:
        for section in TARGET:
            seen=seen_by[section]
            for source,url in RSS.get(lang,{}).get(section,[]):
                try: add_unique(by[section],parse_feed(url,section,lang,source,FALLBACK_HOURS),seen,40)
                except Exception: pass
            for q in queries[section]:
                try: add_unique(by[section],google(section,q,lang,FALLBACK_HOURS),seen,40)
                except Exception: pass

    selected=[]
    for section,n in TARGET.items(): selected.extend(sorted(by[section],key=lambda x:(not x['within_24h'],x.get('published','')),reverse=False)[:n])
    if len(selected)<10:
        pool=[]
        for section in TARGET: pool.extend(x for x in sorted(by[section],key=lambda z:(not z['within_24h'],z.get('published',''))) if x not in selected)
    ...
    selected=sorted(selected[:10],key=lambda x:x.get('published',''),reverse=True)
