...
def context(section,title,text,pt):
    ...
    if not pt:
        if any(x in text for x in ['lng','gas','oil','energy','fuel']): return f'{subject} matters because it may change energy and investment decisions affecting {section}.',f'The development may affect prices, projects, suppliers and supply chains connected to {section}.'
        if any(x in text for x in ['artificial intelligence','technology']): return f'{subject} points to a concrete change in the adoption, regulation or use of technology.',f'The development may change skills demand, productivity, costs and digital opportunities in {section}.'
        if any(x in text for x in ['war','conflict','security','sanction','diplomacy']): return f'{subject} deserves attention because it may change political, diplomatic or security risks.',f'The effects may reach trade, movement, investment and supply chains connected to {section}.'
        if any(x in text for x in ['econom','investment','business','trade','market','inflation','employment']): return f'{subject} helps explain a concrete economic or business change relevant to {section}.',f'The development may affect prices, demand, finance, hiring or supplier opportunities.'
        return f'{subject} is worth following because of its possible economic, institutional or regional effects.',f'The impact will depend on what happens next, but it may affect decisions, costs or opportunities in {section}.'
    if any(x in text for x in ['lng','gás','gas','oil','energy','energia','fuel','combustível']):
        return f'{subject} merece atenção porque pode alterar decisões de energia e investimento em {section}.',f'O desenvolvimento pode afectar preços, projectos, fornecedores e cadeias de abastecimento ligados a {section}.'
    if any(x in text for x in ['artificial intelligence','inteligência artificial','technology','tecnologia']): return f'{subject} é relevante por mostrar uma mudança concreta na adopção, regulação ou uso da tecnologia.',f'O desenvolvimento pode alterar competências procuradas, produtividade, custos e oportunidades digitais em {section}.'
    if any(x in text for x in ['war','guerra','conflict','conflito','security','segurança','sanction','sanções','diplomacia']): return f'{subject} merece acompanhamento porque pode mudar o nível de risco político, diplomático ou de segurança.',f'O efeito pode chegar ao comércio, circulação, investimento e cadeias de abastecimento com ligação a {section}.'
    if any(x in text for x in ['econom','investment','investimento','business','trade','comércio','market','inflation','emprego']): return f'{subject} ajuda a perceber uma mudança económica ou empresarial concreta com relevância para {section}.',f'O desenvolvimento pode reflectir-se em preços, procura, financiamento, contratação ou oportunidades para fornecedores.'
    return f'{subject} merece acompanhamento pelo possível efeito económico, institucional ou regional.',f'O impacto dependerá dos próximos desenvolvimentos, mas poderá afectar decisões, custos ou oportunidades em {section}.'
