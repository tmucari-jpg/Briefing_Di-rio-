...
RSS_PT={
 'Mundo':[('BBC Português','https://feeds.bbci.co.uk/portuguese/international/rss.xml')],
 'África':[('BBC Português','https://feeds.bbci.co.uk/portuguese/topics/africa/rss.xml')],
...
def likely_portuguese(title,desc):
 t=(' '+title+' '+desc).lower()
 return not any(x in t for x in EN_WORDS)
...
def why(sec,t):
 t=t.lower()
 if any(x in t for x in ['lng','gas','oil','energy','energia']):return 'É relevante porque pode mexer com preços, investimento e segurança energética.','Para Moçambique, importa pela exposição ao gás, energia, logística e receitas de exportação.'
 if any(x in t for x in ['artificial intelligence','inteligência artificial','technology','tecnologia']):return 'Pode acelerar ou travar investimento, regulação e adoção tecnológica.','Para Moçambique, interessa por produtividade, emprego digital, serviços públicos e novas oportunidades de negócio.'
 if any(x in t for x in ['war','guerra','conflict','conflito','security','segurança','military','sanction']):return 'Pode alterar riscos geopolíticos, rotas comerciais, energia e custos de financiamento.','Moçambique pode sentir efeitos através de importações, energia, comércio regional e investimento.'
 return 'É uma evolução relevante para a economia, segurança ou ambiente institucional.','Pode afetar investimento, emprego, fornecedores locais, custos ou previsibilidade.'
def parse(url,sec,source,hours,pt=True):
 r=urlopen(Request(url,headers={'User-Agent':'Mozilla/5.0 BriefingDiario/9'}),timeout=25);f=feedparser.parse(r.read());out=[]
 for i in f.entries:
