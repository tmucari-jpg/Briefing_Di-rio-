...
def relevance_score(x):
    ...
    score=0
    for w in ('lng','gás','energia','investimento','economia','emprego','tecnologia','inteligência artificial','segurança','guerra','diplomacia','infraestrutura','mineração'):
        if w in text: score+=1
    if x.get('section')=='Moçambique':
        if any(w in text for w in ('moçambique','mozambique','maputo','cabo delgado','pemba')): score+=3
...
def dedupe(items):
    ...
    for x in sorted(items,key=lambda z:z.get('published',''),reverse=True):
        ...
        for y in out:
            b=norm(y.get('title',''))
            if a==b or SequenceMatcher(None,a,b).ratio()>=0.86:
                duplicate=True; break
        if not duplicate: out.append(x)
    return out
...
def curate(d):
    ...
    if any(counts[s]<n for s,n in TARGET.items()):
        raise SystemExit(f'Qualidade editorial rejeitada: secções insuficientes {counts}.')
    chosen_ids={id(x) for x in selected}
    rest=[x for x in items if id(x) not in chosen_ids]
    selected.extend(rest[:max(0,10-len(selected))])
    selected=sorted(selected[:10],key=lambda x:x['editorial_score'],reverse=True)
    if len(selected)<MIN_NEWS: raise SystemExit(f'Qualidade editorial rejeitada: apenas {len(selected)} notícias válidas.')
    d['items']=selected
    d['section_counts']={s:sum(x['section']==s for x in selected) for s in TARGET}
    d['editorial']='Curadoria automática: relevância + actualidade + qualidade da fonte + deduplicação + validação.'
    d['briefing_intro']={
      'pt':'Bom dia. Este é o Briefing Diário. Hoje, vale a pena acompanhar primeiro os temas com maior impacto potencial em geopolítica, economia, energia, tecnologia e Moçambique.',
      'en':'Good morning. This is the Daily Briefing. Today, focus first on the stories with the strongest potential relevance to geopolitics, the economy, energy, technology and Mozambique.'
    }
    return d
...
pt=json.loads(Path('docs/news-pt.json').read_text(encoding='utf-8'))
Path('docs/news.json').write_text(json.dumps(pt,ensure_ascii=False,indent=2),encoding='utf-8')
print('Editorial OK:',pt['section_counts'],len(pt['items']))